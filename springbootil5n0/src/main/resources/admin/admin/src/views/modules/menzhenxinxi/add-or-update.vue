    ...
    >
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="医师姓名" prop="yishixingming">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="医师姓名"
                  v-model="ruleForm.yishixingming" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yishixingming" label="医师姓名" prop="yishixingming">
                    <span>{{ruleForm.yishixingming}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="专业特长" prop="zhuanyetezhang">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="专业特长"
                  v-model="ruleForm.zhuanyetezhang" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zhuanyetezhang" label="专业特长" prop="zhuanyetezhang">
                    <span>{{ruleForm.zhuanyetezhang}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="值班表" prop="zhibanbiao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.zhibanbiao" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zhibanbiao" label="值班表" prop="zhibanbiao">
                    <span v-html="ruleForm.zhibanbiao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xiangqing = true;
          ...
          if(o=='zhibanbiao'){
            this.ruleForm.zhibanbiao = obj[o];
	    this.ro.zhibanbiao = true;
            continue;
          }
          if(o=='tupian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.zhichengOptions = "医师,主治医师,副主任医师,主任医师".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `menzhenxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.zhibanbiao = this.ruleForm.zhibanbiao.replace(reg,'../../../springbootil5n0/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `menzhenxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.menzhenxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
